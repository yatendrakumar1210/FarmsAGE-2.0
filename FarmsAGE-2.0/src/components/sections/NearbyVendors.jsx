...
const NearbyVendors = () => {
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-12 bg-white font-sans overflow-hidden">
      <div className="bg-[#fbfcff] rounded-[2.5rem] border border-slate-100 p-8 md:p-12 relative overflow-hidden">
        {/* Background blobs */}
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight font-['Outfit']">
                Directly from <span className="text-emerald-600">Local Farms</span>
              </h2>
            </div>
            ...
            <div className="shrink-0">
              {locationStatus === "idle" || locationStatus === "error" ? (
                <button 
                  ...
                  className="flex items-center gap-2.5 bg-emerald-600 text-white px-8 py-3.5 rounded-2xl font-bold shadow-xl shadow-emerald-500/10 hover:bg-emerald-700 hover:-translate-y-0.5 transition-all text-sm active:scale-95"
                >
                  <Navigation2 size={16} />
                  Find Farms Near Me
                </button>
              ...
              {sortedVendors.length > 0 ? (
                sortedVendors.slice(0, 6).map((vendor) => (
                  <motion.div
                    ...
                    className="bg-white rounded-2xl p-4 shadow-sm hover:shadow-xl hover:shadow-emerald-50 transition-all duration-300 border border-slate-100 flex gap-4 items-center group cursor-pointer"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="font-bold text-slate-900 text-base md:text-lg truncate group-hover:text-emerald-600 transition-colors uppercase tracking-tight">{vendor.name}</h3>
                      </div>
                      <p className="text-xs font-medium text-slate-400 mb-2 truncate">{vendor.specialty}</p>
                ...
                >
                  <div className="bg-white/50 border border-dashed border-slate-200 inline-block p-10 rounded-[2.5rem]">
                    ...
                    <h3 className="text-xl font-bold text-slate-800 mb-2">No Farms Nearby Yet</h3>
                    <p className="text-slate-500 max-w-xs mx-auto text-sm font-medium">We're onboarding more farmers daily. Check out our global selection in the meantime!</p>
                  </div>
                </motion.div>
              ) : null}
            </AnimatePresence>
