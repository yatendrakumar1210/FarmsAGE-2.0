...
const MapLocationPicker = ({
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center p-2 sm:p-4 bg-slate-900/70 backdrop-blur-sm animate-fade-in">
      <motion.div
        initial={{ opacity: 0, scale: 0.96, y: 15 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.96, y: 15 }}
        className="bg-white rounded-3xl sm:rounded-[2.5rem] w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden border border-slate-100"
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto flex flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-3/5 h-[300px] sm:h-[360px] lg:h-[500px] relative bg-slate-100 shrink-0">
            {/* Search Bar Overlay */}
            ...
            <button
              ...
              className="absolute bottom-4 right-4 z-[1000] bg-white text-slate-800 px-4 py-2.5 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-2 hover:bg-emerald-50 hover:text-emerald-700 transition-all text-xs font-black"
            >
              {isDetectingGPS ? (
            ...
            {/* Draggable Tip Badge */}
            <div className="absolute bottom-4 left-4 z-[1000] bg-slate-900/80 backdrop-blur-md text-white px-3 py-1.5 rounded-xl text-[10px] font-extrabold flex items-center gap-1.5 shadow-md">
              <span className="text-amber-400">💡</span>
              <span>Tap or drag pin to your exact building</span>
            </div>
          ...
          <div className="flex-1 p-5 sm:p-6 flex flex-col justify-between bg-white space-y-4">
